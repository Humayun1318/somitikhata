...
export default async function PublicPage({
  ...
  const features = t.raw("sections.features") as Array<{
    title: string;
    description: string;
    icon: string;
  }>;
  const faqs = t.raw("sections.faq") as Array<{
    question: string;
    answer: string;
  }>;
  ...
  return (
    <div className="min-h-screen bg-[#F7F8F7] text-[#111827]">
      ...
      <main className="pt-16">
            ...
            <div className="grid items-center gap-10 lg:grid-cols-[1.2fr_0.8fr]">
              <div>
                ...
                <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                  <Link
                    href="/register"
                    className="rounded-xl bg-[#0F6B4F] px-5 py-3 text-center font-semibold text-white hover:bg-[#0C5A42]"
                  >
                    {t("hero.primaryCta")}
                  </Link>
                  <Link
                    href="/login"
                    className="rounded-xl border border-[#E3E6E4] bg-white px-5 py-3 text-center font-semibold text-[#111827] hover:border-[#0F6B4F] hover:text-[#0F6B4F]"
                  >
                    {t("hero.secondaryCta")}
                  </Link>
                </div>
              ...
              <div className="rounded-3xl border border-[#E3E6E4] bg-white p-6 shadow-[0_8px_24px_rgba(0,0,0,0.12)]">
                <div className="rounded-2xl bg-[#F7F8F7] p-5">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="rounded-xl bg-[rgba(15,107,79,0.12)] p-3 text-2xl">
                      💸
                    </div>
                  </div>
                  ...
                  <div className="mt-6 space-y-4">
                    {[
                      ...
                      <div
                        key={String(label)}
                        className="rounded-xl border border-[#E3E6E4] bg-white p-3"
                      >
        ...
        <section
          id="how-it-works"
          className="border-t border-[#E3E6E4] bg-white py-16"
        >
          <PageContainer>
            <div className="mb-10 text-center">
              <p className="text-sm font-semibold uppercase tracking-[0.14em] text-[#0F6B4F]">
                {t("sections.howItWorksLabel")}
              </p>
            ...
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {steps.map((step, index) => (
                <div
                  key={step}
                  className="rounded-2xl border border-[#E3E6E4] bg-[#F7F8F7] p-6"
                >
          ...
          <PageContainer>
            ...
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
              {features.map((feature) => (
                <div
                  key={feature.title}
                  className="rounded-2xl border border-[#E3E6E4] bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.06)]"
                >
                  <div className="mb-4 text-2xl">{feature.icon}</div>
                  <h3 className="mb-2 text-lg font-semibold text-[#111827]">
                    {feature.title}
                  </h3>
                  <p className="text-sm leading-6 text-[#5B6660]">
                    {feature.description}
                  </p>
                </div>
              ))}
          ...
          <PageContainer>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {trustPoints.map((point) => (
                <div
                  key={point}
                  className="rounded-2xl border border-[#E3E6E4] bg-white p-6 text-base font-medium text-[#111827]"
                >
                  {point}
                </div>
              ))}
          ...
          <PageContainer>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {faqs.map((item) => (
                <div
                  key={item.question}
                  className="rounded-2xl border border-[#E3E6E4] bg-white p-6"
                >
                  <h3 className="mb-2 text-lg font-semibold text-[#111827]">
                    {item.question}
                  </h3>
        ...
        <section className="bg-[#1E3A5F] py-16 text-white">
          <PageContainer className="text-center">
            ...
            <div className="mt-8 flex justify-center">
              <Link
                href="/register"
                className="rounded-xl bg-white px-6 py-3 font-semibold text-[#1E3A5F] hover:bg-slate-100"
              >
